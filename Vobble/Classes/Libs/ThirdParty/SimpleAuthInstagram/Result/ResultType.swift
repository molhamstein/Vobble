...
public protocol ResultType {
    ...
    #else
    func analysis<U>( ifSuccess ifSuccess: Value -> U,  ifFailure: Error -> U) -> U
    #endif
...
public extension ResultType {
    ...
    #else
    @warn_unused_result
    public func map<U>( transform: Value -> U) -> Result<U, Error> {
    return flatMap { .Success(transform($0)) }
    }
    #endif
    ...
    #else
    @warn_unused_result
    public func flatMap<U>( transform: Value -> Result<U, Error>) -> Result<U, Error> {
    return analysis(
    ifSuccess: transform,
    ifFailure: Result<U, Error>.Failure)
    }
    ...
    #else
    @warn_unused_result
    public func mapError<Error2>( transform: Error -> Error2) -> Result<Value, Error2> {
    return flatMapError { .Failure(transform($0)) }
    }
    #endif
    ...
    #else
    @warn_unused_result
    public func flatMapError<Error2>( transform: Error -> Result<Value, Error2>) -> Result<Value, Error2> {
    return analysis(
    ifSuccess: Result<Value, Error2>.Success,
    ifFailure: transform)
    }
...
public extension ResultType where Error: ErrorTypeConvertible {
    ...
    #else
    @warn_unused_result
    public func tryMap<U>( transform: Value throws -> U) -> Result<U, Error> {
    return flatMap { value in
    do {
				return .Success(try transform(value))
    }
